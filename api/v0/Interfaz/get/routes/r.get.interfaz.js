...
module.exports = function (app) {
  ...
  app.post("/getMenu2", async (req, res) => {
    try {
      const { body } = req;
      const reqCargo = await User.getCargoByIdAcceso(body);
      const reqMenu =
        reqCargo.cargo === 1
          ? await User.getMenu2(body)
          : await User.getMenuDefecto();
      const menu = JSON.parse(reqMenu.menu);

      const reqTipoAdministracion = await User.getTipoAdministracion(body);

      if (reqTipoAdministracion.estado) {
        const submenus = menu[0].submenus;
        const index = submenus.findIndex(
          (item) => item.nombreMenu === "Recursos"
        );
